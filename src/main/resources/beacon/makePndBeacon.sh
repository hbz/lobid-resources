...
cat *.jsonl | jq -r '. | select(.contribution != null) | .contribution[].agent.id'  | sort | uniq -c | sort -n -r | sed 's#\(.*\)http://d-nb.info/gnd/\(.*\)#\2|\1#g' | tr -d ' '|grep -v null  >> hbzlod-pndbeacon.txt
