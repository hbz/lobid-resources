...
must be executed manually. Then elasticsearch can be lookuped like this:

http://lobid.org/resources/HT002619538

For querying it (only possible residing in the internal hbz net) you can use the
elasticsearch query DSl, like:

http://gaia.hbz-nrw.de:9200/resources/_search?q=HT002619538
...
h1. Internal hbz Instructions

This is for hbz commiter only. It's assumed that you reside in the internal hbz net.

h2. Dependency to lobid-rdf-to-json

The dependency is configured using the pom.xml. As version you use a commit hash of
the "lobid-rdf-to-json repo":https://github.com/hbz/lobid-rdf-to-json. Via "jitpack":https://jitpack.io/ a maven artefact is build on demand and installed to the local maven repo.
...
index is build on saturday. This is the base for the daily incremental updates.
...
to _resources-staging_. This is manually to be tested. If it's approved ok the
index is switched by simply renaming this alias to _resources_ . Both alias' are
...
The three latest indices will be save, also every index with no _-staging_
suffix on the alias. Other indices which start with the same string will be
removed by the index programm after the indexing is finished - the harddisk
is not of endless size ;) .

Note: in production, make always sure that the JSON-LD context is the proper one.
...
productive index', thus not removing any productive index).
