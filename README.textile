...
This may end in an *BUILD SUCCESS* message, meaning that the tested resources don't reflect the changes.
In this case you should add an Aleph-MabXml resource to "hbz01XmlClobs.tar.bz2":https://github.com/hbz/lobid-resources/blob/master/src/test/resources/hbz01XmlClobs.tar.bz2.
that _would_ reflect your changes.
If the build ends with *BUILD FAILURE* the newly generated data isn't equal to the test resources.
This is a good thing beacause you wanted the change. Now you must approve the new outcome. Make a diff on the data:

@diff hbz01.es.nt src/test/resources/hbz01.es.nt@

and, if everything is ok, copy the new data as the new standard to the test resources:

@cp hbz01.es.nt src/test/resources/hbz01.es.nt@

Now you have satisfied the NTriples test, which now build successfully.
As these NTriples are the base for the JSON-LD test this JSON test must fail.
As with the NTriples we have to diff the data. As there are many single files instead of one big file execute this helper script:

@bash jsonDiffAndCopy.sh@

After you followed the instructions of the script, build again:

@mvn clean install@

Now this should result in *BUILD SUCCESS*. Add, commit and push your changes. You're done :)
