...
if [ -s /tmp/jsonschemaValidationOutput.log ]; then
        ...
        mail -s "[sysad] [lobid-resources] Jsonschema validated with ${NUMBER_OF_INVALID_RECORDS} invalid records" "${MAIL_TO}" -a "From: ${MAIL_FROM}" < "head -n5 /tmp/jsonschemaValidationOutput.log"
fi
