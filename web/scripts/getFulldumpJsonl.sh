...
# If bulk download not > 20 GB the filename is prefixed with "broken".
# Removes old data (3 fulldumps kept are enough).
...
function rmOldData {
        if [ $(ls ${FULLDUMP_DIR}/*${FULLDUMP_FNAME_SUFFIX}| wc -l) -gt 3 ]; then
                echo "more than 3 fulldumps - deleting oldest and test again ..."
