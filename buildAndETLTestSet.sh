...
for i in $(cat testIds.txt); do
	if [ ! -f $TARGET/$i ]; then
		curl -s "http://lobid.org/resource?id=$i&format=source" > $TARGET/$i
		grep "^<?xml" $TARGET/$i 2>&1 >/dev/null
		if [ $? -eq 0 ]; then
			echo "Adding new resource $i"
			else
				echo "Ignoring $TARGET/$i, don't seems to be an XML file"
				rm $TARGET/$i
			fi
		fi
done
